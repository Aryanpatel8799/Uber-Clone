...
const CaptainLogin = () => {
   ...
   const handleSubmit=(e)=>{
    ...
    axios.post(`${import.meta.env.VITE_BASE_URL}/captain/login`,captainData).then((res)=>{
      if(res.status===200)
      {
        const data=res.data;
    ...
    return (
      <div className='flex justify-between flex-col h-screen'>
          ...
          <form className='p-7 flex flex-col' onSubmit={handleSubmit} >
             ...
             <p className='text-sm'>Join a fleet? <Link to="/captain-signup" className='text-gray-500'>Register as a Captain</Link></p>     
          </form>
          </div>
          <div className='flex flex-col items-center mb-25'>
             <Link to="/login"><button className='bg-[#58dc0b] text-white py-2 rounded text-lg w-80 text-semibold mt-5'>Sign in as User</button></Link>
          </div>
      </div>
    )
}
